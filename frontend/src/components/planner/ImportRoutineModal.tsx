  ...
  return createPortal(
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-200 dark:border-white/[0.08] bg-slate-50/70 dark:bg-white/[0.02]">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 flex items-center gap-1.5">
              ...
              <span className="font-semibold text-[#223A5E] dark:text-blue-400">{formattedTargetDate}</span>
            </p>
          </div>
        </div>

        {/* WEEKDAY SELECTOR (Pre-selected to matching day, allows switching) */}
        <div className="px-6 py-3 border-b border-slate-200 dark:border-white/[0.06] bg-slate-50 dark:bg-slate-900/60">
          <div className="flex items-center justify-between gap-2 mb-2">
            <span className="text-xs font-semibold text-slate-600 dark:text-slate-400">Select Routine Template:</span>
            {selectedWeekday === defaultWeekday && (
              <span className="text-[10px] font-bold text-[#223A5E] dark:text-blue-400 bg-blue-50 dark:bg-blue-500/10 px-2 py-0.5 rounded-full border border-blue-200 dark:border-blue-500/20">
                ...
                >
                  ...
                  <span
                    className={`text-[9px] px-1.5 py-0.2 rounded-full font-medium ${
                      isActive ? "bg-white/20 text-white" : count > 0 ? "bg-blue-50 dark:bg-blue-500/15 text-[#223A5E] dark:text-blue-300 font-bold" : "bg-slate-100 dark:bg-white/5 text-slate-500 dark:text-slate-500"
                    }`}
                  >
                    {count}
                  </span>
        ...
        {/* BODY CONTENT */}
        <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-white dark:bg-transparent scrollbar-thin scrollbar-thumb-slate-300 dark:scrollbar-thumb-white/10">
          {/* DUPLICATE WARNING ALERT */}
          {duplicateAnalysis.duplicateCount > 0 && (
            <div className="p-4 rounded-2xl bg-amber-500/10 border border-amber-500/25 flex items-start gap-3 text-amber-900 dark:text-amber-200">
          ...
          <div>
            ...
            {templateTasks.length > 0 ? (
                    ...
                    <div
                      ...
                      className={`p-3.5 rounded-2xl border transition-all flex items-center justify-between gap-3 ${
                        isDup
                          ...
                          : "bg-white dark:bg-white/[0.03] border-[#DCE5F0] dark:border-white/[0.08]"
                      }`}
                    >
                      <div className="space-y-1 min-w-0 flex-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="font-bold text-sm text-slate-900 dark:text-white truncate">{task.title}</span>
                          <span
                          ...
                          {task.category && (
                            <span className="text-[10px] px-2 py-0.5 rounded-md bg-slate-100 dark:bg-white/5 text-slate-700 dark:text-slate-300">
                              {task.category}
                            </span>
                          )}
                        </div>
                        ...
                        <div className="flex items-center gap-3 text-xs text-slate-500 dark:text-slate-400 font-mono">
                          ...
                          {task.reminderEnabled && (
                            <span className="inline-flex items-center gap-1 text-[10px] text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-500/10 px-1.5 py-0.2 rounded font-sans">
                      ...
                      <div className="shrink-0">
                        {isDup ? (
                          <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 px-2.5 py-1 rounded-full">
                            Already Scheduled
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 px-2.5 py-1 rounded-full">
              ...
              <div className="py-10 px-4 rounded-2xl border border-dashed border-[#DCE5F0] dark:border-white/10 bg-slate-50/50 dark:bg-white/[0.01] flex flex-col items-center justify-center text-center space-y-2">
                <Info className="w-8 h-8 text-slate-400 dark:text-slate-500" />
                <h4 className="text-sm font-bold text-slate-900 dark:text-white">No tasks in this routine template</h4>
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-200 dark:border-white/[0.08] bg-slate-50 dark:bg-slate-900/60">
          <button
            onClick={onClose}
            className="px-5 py-2.5 rounded-xl text-xs font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/5 border border-slate-200 dark:border-slate-700 transition-colors cursor-pointer bg-white"
          >
            Cancel
          </button>
              ...
              {duplicateAnalysis.duplicateCount > 0 ? (
                <>
                  ...
                  <button
                    ...
                    className="px-4 py-2.5 rounded-xl bg-slate-200 dark:bg-white/10 hover:bg-slate-300 dark:hover:bg-white/15 text-slate-800 dark:text-slate-200 text-xs font-bold transition-colors cursor-pointer"
                  >
